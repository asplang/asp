     ...
     bool fromApp)
{
    ...
    if (!callerAgain)
    {
        ...
        if (ns == 0)
            return AspRunResult_OutOfDataMemory;
        #ifdef ASP_FEATURE_CLASS
        AspDataSetNamespaceIsFunction
            (ns, !AspDataGetFunctionIsClass(function));
        #else
        AspDataSetNamespaceIsFunction(ns, true);
        #endif

        /* For closures, enable access to the chain of nonlocal namespaces. */
        if (closure != 0)
        {
            uint32_t nonlocalNamespaceIndex =
                AspDataGetClosureNonlocalNamespaceIndex(closure);
            AspDataSetNamespaceEnclosingNamespaceIndex
                (ns, nonlocalNamespaceIndex);

            /* Ripple through the chain of nonlocal namespaces, referencing
        ...
        else
        {
            ...
            /* Replace the current local namespace with the function's new
               namespace. */
    ...
    if (callerAgain || AspDataGetFunctionIsApp(function))
    {
        ...
        /* Save any return value generated by the application, generating one
           if none was given. */
...
AspRunResult AspReturnToCaller(AspEngine *engine, AspDataEntry **returnValue)
{
    ...
    if (instanceIndex != 0 && instanceInitialized)
    {
        ...
        {
            #ifdef ASP_DEBUG
            puts("Initialization function must return None");
            #endif
            return AspRunResult_InvalidReturnValue;
        }
